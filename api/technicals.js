// GET /api/technicals?symbol=FER.MC → RSI(14), SMA(20/50/200), EMA + a series with overlays.
...
export default async function handler(req, res) {
  ...
  try {
    const data = await withCache(`tech:${symbol}`, 60 * 30, async () => {
      ...
      const series = pts.map((p, i) => ({ t: p.date.toISOString().slice(0, 10), c: p.close, sma20: sma20[i], sma50: sma50[i], sma200: sma200[i], rsi: rsi[i] })).slice(-300);
